...
export const fetchFolderTemplatesByPath = async (path) => {
  const { configs } = useStore.getState();
  const api = getApi3Instance();
  const endpoint = configs?.FETCH_FOLDER_TEMPLTES_BY_PATH_URL_ENDPOINT+'?path='+path
  try{
    const response = await api.get(endpoint)
    console.log('response from fetchFolderTemplatesByPath')
    return response;
  } catch (error) {
    console.error("upload error:", error);
    return new Error(error.message || "Something went wrong")
...
export const fetchClientFolders = async ({ search = "%", page = 1, baseUrl }) => {
  const { configs } = useStore.getState();
  const api = getApi3Instance();
  const REPO_NAME = getRepoName();
  const encodedSearch = encodeURIComponent(search);
  console.log(encodedSearch, 'search parama')
  console.log(baseUrl, 'clientsList base url from api file')
...
export const fetchFullFolderTree = (rawPath = "", baseUrl) => {
  const { configs } = useStore.getState();
  const api = getApi3Instance();
  const REPO_NAME = getRepoName();
  console.log(baseUrl, 'clienturl base url from api file')
