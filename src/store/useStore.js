  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        })), // state of client-folder-upload-tab
        ...
        getFolderTemplatesByPath: async () => {
          const selectedFolderNode = get().selectedFolderNode
          const setFolderTemplatesList = get().setFolderTemplatesList
          const path = selectedFolderNode?.path
          // path:"\\01 Client Documents\\B\\Baskin Robins Investment Ltd.\\FO Restricted\\A01. Accounting\\G01. General Ledger\\Client Questionnaires and CIF"
          try{
            const encodedPath = encodeURIComponent(path || "")
            const res = await fetchFolderTemplatesByPath(encodedPath)
            console.log(res, "foldertempaltesresponseeee")
            setFolderTemplatesList(res?.data)
          } catch(error){
