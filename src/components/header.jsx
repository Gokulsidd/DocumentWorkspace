...
const HeaderContent = () => {
  ...
  >
    <Button
      ...
      disabled={!selectedDocumentId || uploadSource === 'clientFolder'}
      onClick={() => setIsDropdownOpen(!isDropdownOpen)}
      onMouseEnter={() => isDropdownOpen && setIsDropdownOpen(true)}
      className="h-9 w-9 p-0 text-muted-foreground/70 hover:text-muted-foreground hover:bg-gray-100/50 transition-colors focus-visible:ring-1 focus-visible:ring-gray-200"
    >
      <TooltipProvider delayDuration={300}>
        <Tooltip>
    ...
    {isDropdownOpen && (
      <div 
        className="absolute right-0 mt-0 w-48 rounded-lg border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800 z-50 p-2"
        onMouseEnter={(e) => e.stopPropagation()}
        onMouseLeave={(e) => e.stopPropagation()}
      >
        <div
          onClick={handleSelectAll}
          className="p-2 text-sm flex items-center gap-2 text-muted-foreground font-medium hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700/50 cursor-pointer rounded"
        >
          <input
            type="checkbox"
            ref={(el) => {
            ...
            onChange={() => {}}
            className="h-3 w-3 cursor-pointer accent-gray-600/50 pointer-events-none"
          />
          Select All
        </div>
...
const Header = () => {
  const pathName = usePathname();
  const noHeaderPages = ["/metadata/", "/bulk-upload/"]
